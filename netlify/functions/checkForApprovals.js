...
exports.handler = async () => {
  try {
    ...
    if (welcomeNotSent.length === 0) {
      console.log(`No users with "welcomeSent = false" - terminating.`);
      return { statusCode: 200, body: 'No welcome emails to send' };
    } else if (Object.keys(enabledUsers).length === 0) {
      console.log(`No enabled users - terminating.`);
      ...
      console.log(
        `Sent welcome emails and updated Firestore for ${welcomeProms.length} user(s).`
      );
      return {
        statusCode: 200,
        body: `Sent welcome emails to ${welcomeProms.length} user(s)`,
      };
    }
  } catch (err) {
